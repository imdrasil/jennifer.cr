...
describe Jennifer::QueryBuilder::Join do
  describe "#to_sql" do
    it "includes table name" do
      Factory.build_join.to_sql.should match(/ tests ON /)
    end

    it "calls to_sql on @on" do
      c = Factory.build_criteria
      Factory.build_join(on: c).to_sql.should match(/#{c.to_sql}$/)
    end

    context "left" do
      it "starts with left" do
        Factory.build_join(type: :left).to_sql.should match(/^LEFT JOIN/)
      end
    end

    context "right" do
      it "starts with right" do
        Factory.build_join(type: :right).to_sql.should match(/^RIGHT JOIN/)
      end
    end

    context "inner" do
      it "starts with pure join" do
        Factory.build_join.to_sql.should match(/^JOIN/)
      end
    end
  end
